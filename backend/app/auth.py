...
async def upload_item_image(item_id: int, file: UploadFile = File(...)):
    try:
        content = await file.read()
        cursor.execute("UPDATE items SET item_image = %s WHERE itemid = %s", (psycopg2.Binary(content), item_id))
        conn.commit()
...
def get_item_image(item_id: int):
    cursor.execute("SELECT item_image FROM items WHERE itemid = %s", (item_id,))
    row = cursor.fetchone()
    if row and row[0]:
        return Response(content=row[0], media_type="image/jpeg")
